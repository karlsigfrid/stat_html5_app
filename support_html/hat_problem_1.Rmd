...
We start small with 3 people who randomly take a hat. It can be their own hat or the hat that belongs to one of the other two people. We decide that the variable $Y$ is the number of people who get their own hat, which means that $E[Y]$ is the expected number of people who get their own hat.
...
In order for all three to get their own hat (which means that $Y=3$) you can think like this: The first person must take her own hat, and the chance of that is one in three. Then the second person must also find his hat, which is a one-in-two chance since only two people are left with their hats. If both person 1 and person 2 find their hats the third person also gets her hat, since this is the only hat left. Therefore, we know that
...
* Person 1 gets her hat while the other two take each others hats.
...
Each of these events has a $\cfrac{1}{6}$ chance of happening (check for yourself), so

$P(Y=1)=\cfrac{1}{6}+\cfrac{1}{6}+\cfrac{1}{6} =\cfrac{1}{2}$

We ignore, $P(Y=0)$, since it will be multiplied with zero and disappear.
...
We still have the same three people with the same problem. We also have our variable $Y$ for the number of people who get their own hat, but we will also introduce the indicator variable $X_1, X_2, X_3$. $X_1=1$ if person 1 gets her own hat and otherwise $X_1=0$ Same for $X_2$ and person 2, etc.

$Y = X_1+X_2+X_3 = \sum_{i=1}^3X_i$

What's the point of this, then? The point is that when we calculate $E[Y]$, we can write it as $E[Y]=E[X_1+X_2+X_3]$. This is useful because the expected value of a sum is the sum of the expected values, so

$E[Y]=E[X_1+X_2+X_3] = E[X_1]+E[X_2]+E[X_3]$
 
We just turned the bigger problem into three smaller problems.

$E[X_1]$ is easy to calculate. $X_1=1$ is 1 if person 1 gets her hat and otherwise 0. The probability $P(X_1=1)=\cfrac{1}{3}$ since it is a simple random choice between three hats. Therefore $P(X_1=0)=1-\cfrac{1}{3}=\cfrac{2}{3}$, which gives that
...
$E[X_2]$ is a bit trickier, but we can manage that too. Person 2 gets his hat if first person 1 does not take person 2's hat (probability 2/3) and then person 2 takes his hat (probability 1/2), so the total probability of this happening is $P(X_2=1)=\cfrac{2}{3}\cfrac{1}{2}=\cfrac{1}{3}$.
...
Finally, person 3 gets her hat if the first two people did not get her hat and then she gets her own hat. The first two people have a 2/3 probability of taking person 3's hat, so the probability that they leave it for her is 1/3, so $P(X_3=1)=\cfrac{1}{3}$ and

$E[X_2]=P(X_3=1)=\cfrac{1}{3}$
...
Person one one picks one of N hats randomly, so clearly $P(X_1=1)=\cfrac{1}{N}$.

If person 2 should get his hat two things must happen after each other. First person 1 must not take person 2's hat. The probability of this is $\cfrac{N-1}{N}$ since there are $N-1$ hats that don't belong to person 2. Then person 2 must pick his own hat out of the $N-1$ hats that remain after person 1 took hers. The probability of this is $\cfrac{1}{N-1}$. Therefore, the probability that person 2 gets his own hat is
...
You probability see the pattern here. Each person, regardless of how many people take a hat before them gets their own hat with probability

$\cfrac{N-i+1}{N} \cdot \cfrac{1}{N-i+1}=\cfrac{1}{N}$

Replace $i$ with 1,2, or 3 and you see that you get the formulas above.

For the last person $i=N$, which gives

$\cfrac{N-N+1}{N} \cdot \cfrac{1}{N-N+1}=\cfrac{1}{N} \cdot \cfrac{1}{1}=\cfrac{1}{N}$

We have shown that $P(X_i=1)=\cfrac{1}{N}$ for all $i$, and therefore $E[X_i]=\cfrac{1}{N}$ for all $i$.
...
On the exam you wouldn't write this much. The last part with $N$ hats would be enough. The examples with three hats are there in an attempt to make the problem easier to understand.
